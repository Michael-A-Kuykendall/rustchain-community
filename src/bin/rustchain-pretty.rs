...
#[command(
    ...
    long_about = "RustChain provides AI agent execution with safety validation and monitoring."
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
async fn handle_init(name: Option<&str>) {
    ...
    step("INFO", &format!("Initializing RustChain project: {}", project_name));
