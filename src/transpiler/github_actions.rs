...
impl GitHubActionsParser {
    ...
    pub async fn parse_string(content: &str) -> Result<Mission> {
        ...
        Self::convert_workflow_to_mission(workflow).await
    }
    ...
    async fn convert_workflow_to_mission(workflow: GitHubWorkflow) -> Result<Mission> {
        ...
        for (job_id, job) in &workflow.jobs {
            let job_steps = Self::convert_job_to_steps(job_id, job, &mut context)?;
    ...
    fn convert_job_to_steps(job_id: &str, job: &Job, context: &mut TranspilationContext) -> Result<Vec<MissionStep>> {
        let mut steps = Vec::new();
        ...
        for (step_index, step) in job.steps.iter().enumerate() {
            let rustchain_step = Self::convert_step_to_mission_step(
                job_id, 
                step_index, 
                step, 
                job, 
    ...
    fn convert_step_to_mission_step(
        job_id: &str,
        step_index: usize,
        step: &Step,
        job: &Job,
