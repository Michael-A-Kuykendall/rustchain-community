...
impl ExportEngine {
    ...
    pub fn export_to_github_actions(mission: &Mission, config: &ExportConfig) -> Result<String> {
        ...
        for (index, step) in mission.steps.iter().enumerate() {
            let gh_step = Self::convert_step_to_github_actions(step, config)?;
    ...
    pub fn export_to_terraform(mission: &Mission, config: &ExportConfig) -> Result<String> {
        ...
        for step in &mission.steps {
            let tf_resource = Self::convert_step_to_terraform(step, config)?;
    ...
    pub fn export_to_kubernetes(mission: &Mission, config: &ExportConfig) -> Result<String> {
        ...
        for step in &mission.steps {
            let shell_command = Self::convert_step_to_shell_command(step)?;
    ...
    pub fn export_to_jenkins(mission: &Mission, config: &ExportConfig) -> Result<String> {
        ...
        for step in &mission.steps {
            ...
            let command = Self::convert_step_to_shell_command(step)?;
    ...
    pub fn export_to_docker_compose(mission: &Mission, config: &ExportConfig) -> Result<String> {
        ...
        for step in &mission.steps {
            let cmd = Self::convert_step_to_shell_command(step)?;
            commands.push(cmd);
        }
    ...
    fn convert_step_to_github_actions(step: &MissionStep, _config: &ExportConfig) -> Result<String> {
    ...
    fn convert_step_to_terraform(step: &MissionStep, _config: &ExportConfig) -> Result<String> {
    ...
    fn convert_step_to_shell_command(step: &MissionStep) -> Result<String> {
        match step.step_type {
            StepType::Command => {
                if let Some(command) = step.parameters.get("command") {
                    let cmd = command.as_str().unwrap_or("");
...
mod tests {
    ...
    #[test]
    fn test_convert_step_to_shell_command() {
        ...
        let result = ExportEngine::convert_step_to_shell_command(&command_step);
        assert!(result.is_ok());
        ...
        let result = ExportEngine::convert_step_to_shell_command(&noop_step);
        assert!(result.is_ok());
