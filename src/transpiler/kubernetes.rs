...
impl KubernetesParser {
    ...
    fn create_resource_step(resource: &KubernetesResource, step_id: &str) -> Result<MissionStep> {
        let step_type = Self::step_type_for(&resource.kind);
    ...
    fn step_type_for(kind: &str) -> StepType {
        match kind {
...
spec:
  replicas: 2
    ...
    fn test_step_type_mapping() {
        assert!(matches!(KubernetesParser::step_type_for("Deployment"), StepType::Http));
        assert!(matches!(KubernetesParser::step_type_for("ConfigMap"), StepType::CreateFile));
        assert!(matches!(KubernetesParser::step_type_for("Job"), StepType::Command));
        assert!(matches!(KubernetesParser::step_type_for("Service"), StepType::Http));
    }
