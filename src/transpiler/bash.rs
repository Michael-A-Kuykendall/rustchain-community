...
impl BashParser {
    ...
    fn create_command_steps(command: &BashCommand, step_counter: &mut usize) -> Result<Vec<MissionStep>> {
        ...
        let main_step = MissionStep {
            ...
            step_type: Self::step_type_for(&command.command),
            depends_on: None,
    ...
    fn step_type_for(command: &str) -> StepType {
        match command {
            // File operations
...
curl -X POST http://api.example.com/data
    ...
    fn test_command_step_type_mapping() {
        assert!(matches!(BashParser::step_type_for("touch"), StepType::CreateFile));
        assert!(matches!(BashParser::step_type_for("mkdir"), StepType::CreateFile));
        assert!(matches!(BashParser::step_type_for("rm"), StepType::DeleteFile));
        assert!(matches!(BashParser::step_type_for("curl"), StepType::Http));
        assert!(matches!(BashParser::step_type_for("ls"), StepType::Command));
    }
